...
export async function GET(request: NextRequest) {
  try {
    ...
    const recommendations = await generateAIRecommendations(investments || [], profile)
...
async function generateAIRecommendations(investments: any[], profile: any) {
  ...
  try {
    const recommendationsText = await generateInvestmentRecommendations({
      ...
      investmentGoals: profile?.investment_goals || ["wealth building", "retirement planning"],
    })
...
function generateFallbackRecommendations(investments: any[], totalInvested: number) {
  const recommendations = []
  ...
  if (investments.length < 3 && totalInvested > 50000) {
    recommendations.push({
      ...
      description: "Consider adding different asset classes to reduce risk and improve returns.",
      priority: "high",
  ...
  if (totalInvested < 100000) {
    recommendations.push({
      ...
      description: "Consider increasing your monthly investment to accelerate wealth building.",
      priority: "medium",
