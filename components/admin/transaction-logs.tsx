...
export function TransactionLogs({ logs: initialLogs }: TransactionLogsProps) {
  ...
  const [filteredLogs, setFilteredLogs] = useState<TransactionLog[]>(initialLogs)
  ...
  const filterLogs = () => {
    ...
    if (methodFilter !== "all") {
      filtered = filtered.filter((log) => log.http_method === methodFilter)
    }
  ...
  const generateAISummary = async () => {
    ...
    const commonErrors = errorLogs.reduce(
      (acc, log) => {
    ...
    const summary = `Analysis of ${errorLogs.length} errors:\n\nTop Issues:\n${topErrors
      .map(([key, count]) => {
  ...
  const getStatusBadge = (statusCode: number) => {
    if (statusCode < 300) return <Badge className="bg-green-100 text-green-800">Success</Badge>
    if (statusCode < 400) return <Badge className="bg-yellow-100 text-yellow-800">Redirect</Badge>
    if (statusCode < 500) return <Badge className="bg-red-100 text-red-800">Client Error</Badge>
  ...
  return (
    <div className="space-y-6">
        ...
        <CardContent className="space-y-4">
          ...
          {aiSummary && (
            <div className="bg-purple-50 border border-purple-200 rounded-lg p-4">
              ...
              <pre className="text-sm text-purple-700 whitespace-pre-wrap">{aiSummary}</pre>
            </div>
          )}
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Transaction Logs ({filteredLogs.length} records)</CardTitle>
        </CardHeader>
        <CardContent>
              ...
              <tbody>
                {filteredLogs.map((log) => (
                  <tr key={log.id} className="border-b hover:bg-gray-50">
                    ...
                    <td className="p-2">{log.email || "Anonymous"}</td>
                    <td className="p-2">{getMethodBadge(log.http_method)}</td>
                    <td className="p-2 font-mono text-xs">{log.endpoint}</td>
                    <td className="p-2">{getStatusBadge(log.status_code)}</td>
                    <td className="p-2 max-w-xs truncate">{log.error_message || "-"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
