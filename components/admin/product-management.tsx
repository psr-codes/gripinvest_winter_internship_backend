  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const productData = {
        ...
        minimum_investment: Number.parseFloat(formData.minimum_investment),
        description: formData.description,
      }

      if (editingProduct) {
        const { error } = await supabase.from("investment_products").update(productData).eq("id", editingProduct.id)

        if (error) throw error
      } else {
        const { error } = await supabase.from("investment_products").insert([productData])
    ...
    try {
      const response = await fetch("/api/ai/product-description", {
        ...
        body: JSON.stringify({
          ...
          minInvestment: formData.minimum_investment,
        }),
      })
      ...
      <Card>
        <CardHeader>
          <div className="flex justify-between items-center">
            <CardTitle>Investment Products ({products.length})</CardTitle>
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) => setFormData({ ...formData, annual_yield: e.target.value })}
                        required
                      />
                    </div>
                    <div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b">
                  ...
                  <th className="text-left p-2">Min Investment</th>
              ...
              <tbody>
                {products.map((product) => (
                  <tr key={product.id} className="border-b hover:bg-gray-50">
                    ...
                    <td className="p-2">{getTypeBadge(product.product_type)}</td>
                    <td className="p-2">{product.annual_yield}%</td>
                    <td className="p-2">{getRiskBadge(product.risk_level)}</td>
                    <td className="p-2">₹{product.minimum_investment?.toLocaleString() || "0"}</td>
                    <td className="p-2">{product.tenure_months} months</td>
                    <td className="p-2">
