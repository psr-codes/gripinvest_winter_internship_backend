...
export function TestCases() {
  ...
  const [runningTests, setRunningTests] = useState<Set<string>>(new Set(["6"]))
  ...
  const runAllTests = () => {
    const pendingTests = testCases.filter((test) => test.status === "pending").map((test) => test.id)
          ...
          <div className="space-y-4">
            <div>
              <div className="flex justify-between items-center mb-2">
                ...
                <span className="text-sm text-gray-600">{overallCoverage}%</span>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 text-center">
              ...
              <div className="bg-yellow-50 p-4 rounded-lg">
                <div className="text-2xl font-bold text-yellow-600">
                  {testCases.filter((t) => t.status === "running" || runningTests.has(t.id)).length}
                </div>
                <div className="text-sm text-yellow-600">Running</div>
              </div>
              <div className="bg-gray-50 p-4 rounded-lg">
                <div className="text-2xl font-bold text-gray-600">
                  {testCases.filter((t) => t.status === "pending" && !runningTests.has(t.id)).length}
                </div>
          ...
          <div className="space-y-4">
            {testCases.map((test) => (
              <div key={test.id} className="border rounded-lg p-4 hover:bg-gray-50">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-4">
                    <div className="text-right">
                      ...
                      <Progress value={test.coverage} className="w-20 h-2" />
                    </div>
                ...
                <div className="mt-2 text-xs text-gray-500">Last run: {test.lastRun}</div>
              </div>
            ))}
          </div>
        </CardContent>
