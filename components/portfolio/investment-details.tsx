  ...
  return (
    <Card>
      <CardHeader>
        ...
        <p className="text-sm text-gray-600">Complete list of your investments with current performance</p>
      </CardHeader>
      <CardContent>
        {investments.length === 0 ? (
          <div className="text-center py-8">
            <p className="text-gray-600 mb-4">No investments found</p>
            <Button className="bg-green-600 hover:bg-green-700">Start Investing</Button>
          </div>
        ) : (
          <div className="space-y-4">
            {investments.map((investment) => {
              const returns = investment.current_value - investment.invested_amount
              const returnPercentage = ((returns / investment.invested_amount) * 100).toFixed(1)
                ...
                >
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      <h3 className="font-medium text-gray-900">{investment.product_name}</h3>
                      <Badge className={getTypeColor(investment.product_type)}>{investment.product_type}</Badge>
                      <Badge className={getRiskColor(investment.product_type)}>
                        {getRiskLabel(investment.product_type)}
                      </Badge>
                    </div>
                  ...
                  <div className="text-right">
                    ...
                    <div className="font-medium">{formatCurrency(investment.invested_amount)}</div>
                  </div>
                  ...
                  <div className="text-right ml-6">
                    ...
                    >
                      ...
                      <span>
                        {isPositive ? "+" : ""}
                        {formatCurrency(returns)} ({returnPercentage}%)
                      </span>
                    </div>
                  </div>
                </div>
