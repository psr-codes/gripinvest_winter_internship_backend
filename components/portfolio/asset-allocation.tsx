...
export function AssetAllocation({ investments }: AssetAllocationProps) {
    ...
    const allocationData = investments.reduce((acc, investment) => {
        const existing = acc.find(
            (item) => item.type === investment.investment_type
        );
        if (existing) {
    ...
    const totalValue = allocationData.reduce(
                ...
                <CardContent>
                    <div className="text-center py-8">
                        <p className="text-gray-600">
                            No investments to show allocation
                        </p>
                    </div>
                </CardContent>
            </Card>
                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <p className="font-medium">
                                            {getTypeLabel(item.name)}
                                        </p>
                                        <p className="text-sm text-gray-600">
                                            {item.count} investment(s)
                                        </p>
                                    </div>
                                </div>
